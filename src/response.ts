...
const getResmap = (
	...
	const resmapNormal: [
		...
		[
			/^(うにゅう、|うにゅう[くさた]ん、|うにゅう[ちに]ゃん、)?(.{1,300})[をに]([燃萌も]やして|焼いて|煮て|炊いて|沸か[せし]て|溶かして|凍らせて|冷やして|冷まして|通報して|火を[付つ]けて|磨いて|爆破して|注射して|打って|駐車して|停めて|潰して|縮めて|伸ばして|広げて|ど[突つ]いて|[踏ふ]んで|捌いて|裁いて|出して|積んで|重ねて|握って|触って|祝って|呪って|鳴らして|詰めて|梱包して|囲んで|囲って|詰んで|漬けて|[踊躍]らせて|撃って|蒸して|上げて|アゲて|ageて|下げて|サゲて|sageて|導いて|支えて|応援して|増やして|包囲して|沈めて|願って|祈って|直して|秘めて)[^るた]?$/su,
			res_fire
		]
	];
	const resmapReply: [
...
const res_iiyo = (event: NostrEvent, mode: Mode): [string, string[][]] => {
	...
	if (/(かわいい|可愛い)の?か?(？|\?)$/.test(event.content)) {
		...
		/何|なに|誰|だれ|どこ|いつ|どう|どんな|どの|どっち|どちら|どれ|いくら/.test(event.content)
	) {
...
const getTagsFav = (event: NostrEvent): string[][] => {
	const tagsFav: string[][] = [
		...event.tags.filter(
			(tag: string[]) =>
				tag.length >= 2 && (tag[0] === 'e' || (tag[0] === 'p' && tag[1] !== event.pubkey))
		),
		['e', event.id, '', ''],
		['p', event.pubkey, ''],
		['k', String(event.kind)]
