...
const getResmap = (
  ...
  const resmapNormal: [
    ...
    [
      /^(うにゅう、|うにゅう[くさた]ん、|うにゅうちゃん、)?(.{1,300})[をに]([燃萌も]やして|焼いて|煮て|炊いて|沸か[せし]て|溶かして|凍らせて|冷やして|通報して|火を[付つ]けて|磨いて|爆破して|注射して|打って|駐車して|停めて|潰して|縮めて|伸ばして|広げて|ど[突つ]いて|[踏ふ]んで|捌いて|裁いて|出して|積んで|握って|触って|祝って|呪って|鳴らして|詰めて|梱包して|囲んで|囲って|詰んで|漬けて|[踊躍]らせて|撃って|蒸して|上げて|アゲて|ageて|下げて|サゲて|sageて|導いて|支えて)[^るた]?$/su,
      res_fire,
    ],
  ];
  const resmapReply: [
    ...
    [/占って|占い/, res_uranai],
    [
      /(^|\s+)(うにゅう、|うにゅう[くさた]ん、|うにゅうちゃん、)?(\S+)の(週間)?天気/,
      res_tenki,
    ],
    [/(^|\s+)うにゅう、自(\S+)しろ/, res_aura],
    [
      /(^|\s+)(うにゅう、|うにゅう[くさた]ん、|うにゅうちゃん、)?(.+)を絵文字にして$/u,
      res_emojinishite,
    ],
...
const mode_normal = async (
  ...
  //自分への話しかけはreplyで対応する
  if (
    /^(うにゅう、|うにゅう[くさた]ん、|うにゅうちゃん、)/.test(event.content)
  ) {
    return null;
  }
...
const res_fire = (
  ...
  if (/(潰して|縮めて)[^るた]?$/u.test(event.content)) {
    ...
    if (/([踏ふ]んで|捌いて|握って|触って)[^るた]?$/u.test(event.content)) {
      ...
      /(詰めて|梱包して|漬けて|囲んで|囲って)[^るた]?$/u.test(event.content)
    ) {
